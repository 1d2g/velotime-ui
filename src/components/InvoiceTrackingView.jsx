...
export default function InvoiceTrackingView({
  ...
  // Date Editor Modal state
  const [editingDateInvoice, setEditingDateInvoice] = useState(null);
  const [editDateIssued, setEditDateIssued] = useState("");
  const [editDueDate, setEditDueDate] = useState("");
  const [isSavingDates, setIsSavingDates] = useState(false);

  const openDateEditor = (inv) => {
    setEditingDateInvoice(inv);
    setEditDateIssued(
      inv.dateIssued
        ? new Date(inv.dateIssued).toISOString().split("T")[0]
  ...
  const handleSaveCustomDates = async () => {
    if (!editingDateInvoice) return;
    setIsSavingDates(true);
    try {
      const updated = await apiCall(`/api/invoices/${editingDateInvoice.id}`, "PUT", {
        dateIssued: editDateIssued,
        dueDate: editDueDate || null,
      });
      if (onUpdateInvoice) {
        onUpdateInvoice(updated);
      }
      addToast(`Dates updated for ${editingDateInvoice.invoiceNumber}`, "success");
      setEditingDateInvoice(null);
    } catch (err) {
      addToast("Failed to update invoice dates", "error");
    } finally {
  ...
  const enrichedInvoices = useMemo(() => {
    ...
    return invoices.map((inv) => {
      ...
      // Invoices that have not been sent (draft) MUST NOT be marked as overdue
      const isSent = inv.status === "sent" || inv.status === "pending" || inv.status === "overdue";
      ...
      if (inv.dueDate) {
        ...
        if (isSent && !isPaid && dueDays < 0) {
          isOverdue = true;
        } else if (isSent && !isPaid && dueDays >= 0 && dueDays <= 7) {
          isDueSoon = true;
        }
      }
  ...
  const handleUpdateStatus = async (invoiceId, newStatus) => {
    ...
    try {
      ...
      if (newStatus === "sent") {
        ...
        // Automatically recalculate dueDate so an invoice sent today is not expired immediately
        let termDays = 30;
      ...
      const updated = await apiCall(`/api/invoices/${invoiceId}`, "PUT", payload);
      if (onUpdateInvoice) {
        onUpdateInvoice(updated);
      }
      if (activeReminderInvoice && activeReminderInvoice.id === invoiceId) {
        setActiveReminderInvoice((prev) => (prev ? { ...prev, ...payload } : null));
      }
      addToast(
        newStatus === "draft"
          ? "Status updated to NOT SENT"
          : newStatus === "sent"
          ? "Invoice marked as SENT (date set to today)"
    ...
    <div className="flex-1 flex flex-col h-full bg-slate-100 dark:bg-zinc-950 overflow-y-auto p-4 sm:p-8">
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center gap-1.5">
                                {inv.status === "draft" ? (
                                  ...
                                  <span className="text-[11px] font-medium text-slate-600 dark:text-slate-300">
                                    Sent {formatDate(inv.dateIssued)}
                                  </span>
                                )}
                                <button
                                  type="button"
                                  onClick={() => openDateEditor(inv)}
                                  className="inline-flex items-center gap-1 text-[10px] font-semibold text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white px-1.5 py-0.5 border border-slate-200 dark:border-zinc-700 hover:border-slate-400 bg-white dark:bg-zinc-800 transition-colors cursor-pointer shadow-2xs"
                                  title="Edit sent / due dates"
                                >
                              ...
                              {isPaid ? (
                                ...
                                <span className="text-[11px] text-slate-400">
                                  {inv.dueDate ? `Target Due ${formatDate(inv.dueDate)}` : "No due date set"}
                                </span>
                              ) : inv.isOverdue ? (
      ...
      {editingDateInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 w-full max-w-md shadow-2xl p-6 space-y-5 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex justify-between items-start border-b border-slate-200 dark:border-zinc-800 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  <span className="font-bold text-slate-700 dark:text-slate-300 tabular-nums">
                    {editingDateInvoice.invoiceNumber}
                  </span>{" "}
                  • {editingDateInvoice.resolvedClientName}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setEditingDateInvoice(null)}
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <button
                    ...
                    onClick={() => {
                      const today = new Date().toISOString().split("T")[0];
                      setEditDateIssued(today);
                    }}
                    className="text-[11px] font-bold text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white underline cursor-pointer"
                  >
                    Set to Today
                  </button>
                </div>
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 px-3 py-2 text-xs font-bold tabular-nums text-slate-900 dark:text-white outline-none focus:border-slate-500 cursor-pointer"
                />
              </div>
                ...
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="flex items-center gap-1.5 text-[10px] font-bold">
                    <button
                      type="button"
                      onClick={() => {
                        const base = editDateIssued ? new Date(editDateIssued) : new Date();
                        base.setDate(base.getDate() + 15);
                        setEditDueDate(base.toISOString().split("T")[0]);
                      }}
                      className="px-1.5 py-0.5 border border-slate-200 dark:border-zinc-700 hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-600 dark:text-slate-300 cursor-pointer"
                    >
                      +15d
                    </button>
                    <button
            ...
            <div className="flex justify-end items-center gap-2 pt-3 border-t border-slate-200 dark:border-zinc-800">
              <button
                type="button"
                onClick={() => setEditingDateInvoice(null)}
                className="px-3.5 py-2 border border-slate-300 dark:border-zinc-700 hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-700 dark:text-slate-300 text-xs font-bold cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:opacity-90 text-xs font-bold cursor-pointer disabled:opacity-50"
              >
                <Check className="w-3.5 h-3.5" />
                <span>{isSavingDates ? "Saving..." : "Save Dates"}</span>
              </button>
            </div>
          </div>
        </div>
